    ...
    <div class="modal" id="modaladd">
        <div class="modal-dialog" role="document">
            <div class="modal-content modal-content-demo">
                ...
                <form action="<?= base_url() ?>tambah_bimbingan_revisi_skripsi" method="POST" enctype="multipart/form-data">
                    <?= csrf_field() ?>
                    ...
                    <div class="main-chat-list" id="ChatList">
                        <!-- <?php
                                foreach ($dosen_pembimbing as $key2) {
                                    $notif = $db->query("SELECT *, COUNT( * ) AS total FROM tb_bimbingan WHERE status_baca='belum dibaca' AND `from`='$key2->nip' AND `to`='" . session()->get('ses_id') . "' AND kategori_bimbingan=4 GROUP BY `from`")->getResult();
                                ?>
                            <a href="<?= base_url() ?>bimbingan_revisi_skripsi/<?= $key2->nip ?>">
                                <?php if ($notif != NULL) { ?>
                                    <div class="media new">
                                    <?php } else {
                                    ?>
                                ...
                                <?php if ($notif != NULL) { ?>
                                    ...
                                    ?>
                                        ...
                                        <div class="media-body">
                                            ...
                                            <?php
                                            $status_acc_revisi = $db->query("SELECT * FROM tb_acc_revisi WHERE nim='" . session()->get('ses_id') . "' AND nip='" . $key2->nip . "' AND jenis_sidang='skripsi'")->getResult();
        ...
        <div class="col-xl-8 col-lg-7 col-sm-7">
            ...
            <div class="card">
                            ...
                            <script>
                                $(document).ready(function() {
                                    setInterval(function() {
                                        $.get("<?= base_url() . "reload_m_bimbingan_revisi_skripsi/$how" ?>", function(result) {
                            ...
                            <div class="modal" id="modaldel">
                                <div class="modal-dialog" role="document">
                                    <div class="modal-content modal-content-demo">
                                        ...
                                        <form action="<?= base_url() ?>hapus_bimbingan_revisi_skripsi" method="POST" enctype="multipart/form-data">
                                            <?= csrf_field() ?>
                ...
                <div class="main-chat-footer">
                    <form action="<?= base_url() ?>tambah_revisi_bimbingan_skripsi" method="POST" enctype="multipart/form-data">
                        <?= csrf_field() ?>
